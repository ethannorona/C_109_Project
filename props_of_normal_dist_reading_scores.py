...
scores_mean = statistics.mean(reading_scores)
scores_std_deviation = statistics.stdev(reading_scores)
scores_median = statistics.median(reading_scores)
scores_mode = statistics.mode(reading_scores)
...
thin_1_std_deviation = [result for result in reading_scores if result > first_std_deviation_start and result < first_std_deviation_end]
thin_2_std_deviation = [result for result in reading_scores if result > second_std_deviation_start and result < second_std_deviation_end]
thin_3_std_deviation = [result for result in reading_scores if result > third_std_deviation_start and result < third_std_deviation_end]

#Finding the percentage of data in the lists
print("{}% of data for reading scores lies within 1 standard deviation".format(len(thin_1_std_deviation)*100.0/len(reading_scores)))
print("{}% of data for reading scores lies within 2 standard deviation".format(len(thin_2_std_deviation)*100.0/len(reading_scores)))
print("{}% of data for reading scores lies within 3 standard deviation".format(len(thin_3_std_deviation)*100.0/len(reading_scores)))

#Plotting the bell curve graph with lines showing standard deviations 
fig = ff.create_distplot([reading_scores], ["Reading Scores"], show_hist = False)
