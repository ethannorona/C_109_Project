...
scores_mean = statistics.mean(writing_scores)
scores_std_deviation = statistics.stdev(writing_scores)
scores_median = statistics.median(writing_scores)
scores_mode = statistics.mode(writing_scores)
...
thin_1_std_deviation = [result for result in writing_scores if result > first_std_deviation_start and result < first_std_deviation_end]
thin_2_std_deviation = [result for result in writing_scores if result > second_std_deviation_start and result < second_std_deviation_end]
thin_3_std_deviation = [result for result in writing_scores if result > third_std_deviation_start and result < third_std_deviation_end]

#Finding the percentage of data in the lists
print("{}% of data for writing scores lies within 1 standard deviation".format(len(thin_1_std_deviation)*100.0/len(writing_scores)))
print("{}% of data for writing scores lies within 2 standard deviation".format(len(thin_2_std_deviation)*100.0/len(writing_scores)))
print("{}% of data for writing scores lies within 3 standard deviation".format(len(thin_3_std_deviation)*100.0/len(writing_scores)))

#Plotting the bell curve graph with lines showing standard deviations 
fig = ff.create_distplot([writing_scores], ["Writing Scores"], show_hist = False)
